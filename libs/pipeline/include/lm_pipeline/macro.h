...
#define to_str_(name) str(get(var))
...
#define capture_(var, predicate) \
	do { \
		lm_pipeline_start_capture(lm_pipeline__ctx, var); \
		unsigned char lm_pipeline__condition; \
		do { \
			...
			lm_pipeline__condition = (predicate); \
		} while (lm_pipeline__condition == 0); \
		lm_pipeline__suffix.end = lm_pipeline_get_num_tokens(lm_pipeline__ctx); \
		lm_pipeline_rewind( \
			lm_pipeline__ctx, \
			lm_pipeline_get_num_tokens(lm_pipeline__ctx) - lm_pipeline__condition \
		); \
